...
# Fit DLM model
## fit the model
ef.out <- ecoforecastR::fit_dlm(model=list(obs="Biomass",fixed="~ 1 + X + Depth + SeasonalSST"),
                                mean.maine.lobstah)
...
season.maine.lobstah <- aggregate(cbind(Biomass, Depth, SeasonalSST) ~ Year + Season, 
                                maine.lobstah, mean)
...
season.data <- list(y=season.maine.lobstah$Biomass,
