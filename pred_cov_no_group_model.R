...
zones <- sort(unique(d_spr$ZONEID))
...

{
  ...
  for (i in 2:NY)
  {
    for (j in 1:NZ)
    {
      ...
      mu[i,j] <- x[i-1,j] + beta_x * x[i-1,j] + kappa[j] * sst[i,j]
  ...
  #   for (j in 1:NZ)
  #   {
  #     beta_x[i,j] ~ dnorm(mu_beta_x[i,j], tau_beta_x)
  #     mu_beta_x[i,j] <- kappa * sst[i,j]
  #   }
  # }
  #tau_beta_x ~ dgamma(0.01, 0.01)
  beta_x ~ dnorm(0, 0.01)
  ...
  for (j in 1:NZ)
  {
    kappa[j] ~ dnorm(mu_kappa, tau_kappa)
  }
  mu_kappa ~ dnorm(0, 0.01)
  tau_kappa ~ dgamma(0.01, 0.01)
  #kappa ~ dnorm(0, 0.01)
...
#summarize output
MCMCvis::MCMCsummary(fit, excl = c('x', 'x_all', 'ysim', 
                                   'ysim_all'), round = 2)

mn_x_ch <- MCMCvis::MCMCchains(fit, params = 'mn_x')
e_mn_x_ch <- exp(mn_x_ch)


#Mean biomass
setwd("~/Google_Drive/R/NEFI_course/Fish_project/Figs")
pdf('mn_x.pdf', height = 4.5, width = 4.5)
MCMCvis::MCMCplot(e_mn_x_ch, 
                  labels = c('Zone A', 'Zone B',
                             'Zone C', 'Zone D', 
                             'Zone E', 'Zone F',
                             'Zone G'),
                  ref = NULL,
...
setwd("~/Google_Drive/R/NEFI_course/Fish_project/Figs")
pdf('kappa.pdf', height = 4.5, width = 4.5)
MCMCvis::MCMCplot(fit, params = 'kappa',
                  labels = c('Zone A', 'Zone B',
                             'Zone C', 'Zone D', 
                             'Zone E', 'Zone F',
                             'Zone G'),
                  main = 'Kappa - SST effect')
dev.off()

# kappa_ch <- MCMCvis::MCMCchains(fit, params = 'kappa')
# setwd("~/Google_Drive/R/NEFI_course/Fish_project/Figs")
# pdf('kappa.pdf', height = 4.5, width = 4.5)
# plot(density(kappa_ch), lwd = 3, main = '', xlab = '', ylab = '')
...
{
  pdf(paste0('pred_zone', zones[i], '.pdf'), width = 5, height = 5)
  #pdf(paste0('reference.pdf'), width = 5, height = 5)
  #i <- 1
  plot(ftime, ftime, type = 'n', ylim = range(com, na.rm = TRUE),
       ylab = "Biomass", xlab = 'Year', main = paste0('Zone ', zones[i]))
       #ylab = "", xlab = '', main = paste0('Zone ', zones[i]),
  ...
  polygon(cbind(c(time, rev(time), time[1]), 
                c(ys_LCI[time,i], rev(ys_UCI[time,i]), ys_LCI[1,i])), 
          border = FALSE, col = rgb(0,0,1,0.5))
  polygon(cbind(c(ptime, rev(ptime), ptime[1]), 
                c(ys_LCI[ptime,i], rev(ys_UCI[ptime,i]), ys_LCI[1,i])), 
          border = FALSE, col = rgb(0,0,1,0.3))
  
  #CI
  polygon(cbind(c(time, rev(time), time[1]), 
                c(x_LCI[time,i], rev(x_UCI[time,i]), x_LCI[1,i])), 
          border = FALSE, col = rgb(1,0,0,0.5))
  polygon(cbind(c(ptime, rev(ptime), ptime[1]), 
                c(x_LCI[ptime,i], rev(x_UCI[ptime,i]), x_LCI[1,i])), 
          border = FALSE, col = rgb(1,0,0,0.3))
  
  bl <- rgb(0,0,0,0.8)
  gry <- rgb(0.75,0.75,0.75,1)
  lines(time, x_med[time,i], lwd = 3, col = gry) #model mean
  lines(ptime, x_med[ptime,i], lwd = 3, col = bl) #model mean
  
  #actual data
  points(ftime, exp(DATA$y[,i]), pch = '+', col = gry, cex = 1.5)
  #witheld data
  points(to.na, exp(out_wh[,i]), pch = '+', col = bl, cex = 1.5)
  dev.off()
}
...
#coverage plot
setwd("~/Google_Drive/R/NEFI_course/Fish_project/Figs")
pdf('coverage.pdf', height = 4.5, width = 4.5)
par(mfrow = c(1,1))
plot(ch_vec, pin_vec, ylab = 'Prop witheld data in prediction interval',
     xlab = 'Prediction interval', pch = '*', cex = 2, col = 'white')
abline(a = 0, b = 1, col = 'red', lty = 2, lwd = 3)
points(ch_vec, pin_vec, pch = '*', cex = 2)
dev.off()
